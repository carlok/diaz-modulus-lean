...
\date{September 2026\\[2pt]\small Version 1.5, 24 September 2026}
...
$\e^{\pi^{2}}$ and $\e^{i\pi^{3}}$ is transcendental. The smallest open instance of the conjecture,
$\sqrt{(\log2)^{2}+\pi^{2}}$, is paired with $2^{i\log2/\pi}$: at least one of the two is
transcendental.
\end{abstract}
...
$\e^{i/\pi}$ and $\e^{i\pi^{3}}$ is transcendental.
...
\emph{sum}, and every available tool is homogeneous. The homogeneous companion of that question is
settled at transcendence degree one: for non-real $\lambda\in\Logs$ with $\lambda$, $\bar\lambda$
algebraically dependent, $\e^{|\lambda|}$ is transcendental (Roy--Waldschmidt~\cite[Cor.~7.4]{roy-waldschmidt-1997}).
So if $\log2$ and $\pi$ were algebraically dependent, $\exp\sqrt{(\log2)^{2}+\pi^{2}}$ would be
transcendental. Whether $\sqrt{(\log2)^{2}+\pi^{2}}$ is itself algebraic is the inhomogeneous
...
one of $2^{\log_{3}2}$ and $3^{\log_{2}3}$ is. The room can also come from a second candidate:
...
exponentials theorem~\cite{roy-1992}, the proved result closest to the strong four exponentials
conjecture, gives nothing against a generic candidate even with the period added. Diaz observed
...
exponentials with $x=(1,u/\bar u)$ and $y=(\bar u,w,\overline{(u/\bar u)w})$.
...
This is version 1.5 of the note; version 1.0 of 21 September 2026 is the first stable release.
