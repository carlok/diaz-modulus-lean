...
\date{September 2026\\[2pt]\small Stable version 1.0, 21 September 2026}
...
does not see the candidate's arithmetic at all. What is left is three statements, all open
mathematics rather than formalisation, and we say exactly which.
\end{abstract}
...
both halves are open, and they are two of the three statements at which the formal development
stops (Section~\ref{sec:what}).
...
machine-checked reduction, to one of exactly three statements, and nothing else in it is open:
\begin{enumerate}[label=\textup{(\roman*)}]
  \item the real half of $(S)$: for real algebraic $\gamma\neq0$, $\e^{-i\gamma/\pi}$ is
        transcendental;
  \item the imaginary half of $(S)$: for real algebraic $\beta\neq0$, $\e^{\beta/\pi}$ is
        transcendental;
  \item the statement $(\mathrm{NT})$ of Proposition~\ref{prop:loop}, which is equivalent to the
        conjecture itself.
\end{enumerate}
All three follow from Conjecture~\ref{conj:sfe}. None of them is a formalisation problem.
...
This is the stable version of the note. Every row of the table below was checked against the
platform by script on 21 September 2026, and the mirrored project held all 166 proved results of
the development at that date.
