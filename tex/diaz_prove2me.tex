...
\date{September 2026\\[2pt]\small Version 1.9, 25 September 2026}
...
two of the numbered statements below have been checked by machine, four of them with Baker's theorem
on linear forms in logarithms taken as a hypothesis, and the two exceptions are flagged
where they appear; Appendix~A records where each proof lives, so that any claim here can be
...
Three classical inputs are used without proof. \emph{Hermite--Lindemann}: for algebraic
$a\neq0$, $\e^{a}$ is transcendental. \emph{Transcendence of $\pi$}, and hence of $\pi^{2}$.
\emph{Baker's theorem}, in its inhomogeneous form: if $\lambda_{1},\dots,\lambda_{n}\in\Logs$ are
linearly independent over $\Q$, then $1,\lambda_{1},\dots,\lambda_{n}$ are linearly independent over
$\Alg$. Unlike the first two, it has no machine-checked proof here: the formal statements of
Proposition~\ref{prop:no-deriv}, Theorems~\ref{thm:line} and~\ref{thm:saturation} and
Corollary~\ref{cor:api} take it, or its instance at $u$ and $\bar u$, as an explicit hypothesis.
...
For $u$ with $\operatorname{Im}u\in\pi\Z\setminus\{0\}$, and with no assumption on $\operatorname{Re}u$,
the sharpened bound holds across the entire orbit $\{qu: q\in\Q^{\times}\}$, that is,
$\lvert qu\rvert^{2}>\pi^{2}/m^{2}$ for every $q\in\Q^{\times}$ and every $m\ge1$ with
$\bigl(\e^{qu}/\overline{\e^{qu}}\bigr)^{m}=1$, if and only if $\operatorname{Re}u\neq0$.
\end{proposition}
...
Corollary~\ref{cor:translate} has no formal counterpart of its own, and Appendix~\ref{app:certs} marks it so. It is worth reading twice. The condition it produces was introduced
...
Every numbered statement above but two has been checked in a formal development, four of them with
Baker's theorem as a hypothesis, in Lean~4 against a pinned revision of its mathematical library. The table records where each one lives, so
...
This is version 1.9 of the note; version 1.0 of 21 September 2026 is the first stable release.
...
theirs, and each is attributed in the text. Version 1.7 adds rows for the results of that reading that are
now machine-checked, and version 1.8 two more: Diaz's Proposition~1 and Waldschmidt's Consequence~1.6.
Version 1.9 corrects this appendix against the formal statements. Four rows now say that their proofs
take Baker's theorem as a hypothesis, and Section~1 lists it as a third input used without proof;
Corollary~\ref{cor:translate} is marked as not formalised; and Proposition~\ref{prop:quant-orbit}
is restated as its formal counterpart proves it.
Every row of the table below was checked against the platform by script on 25 September 2026, and
the mirrored project held all 254 proved results of the development at that date.
...
A \textbf{Proved} row means the statement itself has a machine-checked proof. A \textbf{Proved, assuming Baker}
row means the proof is machine-checked with Baker's theorem (Section~1), or an instance of it, taken as an
explicit hypothesis, because the library used contains no proof of Baker's theorem. An \textbf{Open}
...
Corollary~\ref{cor:translate} and Proposition~\ref{prop:loop} are flagged that way. The row of
Proposition~\ref{prop:loop} needs a word: the identifier given
...
Cor.~\ref{cor:translate} & Not formalised & none; it follows from the identity \nolinkurl{Diaz.period_plane_norm} \\
...
\S\ref{sec:quant}, Waldschmidt's remark & Proved & \nolinkurl{DiazModulus.log_pair_square_ratio_transcendental}, \nolinkurl{DiazModulus.log_two_pi_dependent_forces_transcendence} \\
\S\ref{sec:quant}, Diaz's Proposition~1 & Proved & \nolinkurl{DiazModulus.log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq} \\
...
\S\ref{sec:boundary}, the $2\times2$ conjecture & Proved & \nolinkurl{DiazModulus.diaz_of_two_by_two_determinant_conjecture} \\
\S\ref{sec:boundary}, the companion $\e^{|\lambda|}$ & Proved & \nolinkurl{DiazModulus.exp_abs_transcendental_of_conj_algebraic}, \nolinkurl{DiazModulus.exp_abs_log_two_add_i_pi_transcendental} \\
...
After Thm~\ref{thm:S-of-sfe}, Consequence~1.6 & Proved & \nolinkurl{DiazModulus.div_not_mem_logAlgTilde_of_sfe} (it states Consequence~1.7, which contains~1.6) \\
\S\ref{sec:what}, item (2) & Proved & \nolinkurl{DiazModulus.diaz_of_strong_five_exponentials}, \nolinkurl{DiazModulus.recip_pi_not_log_of_strong_five_exponentials} \\
...
Prop.~\ref{prop:no-deriv} & Proved, assuming Baker & \nolinkurl{DiazModulus.no_first_order_arithmetic_operator} (hypothesis: $1,u,\bar u$ are linearly independent over $\Alg$) \\
...
Thm~\ref{thm:line} & Proved, assuming Baker & \nolinkurl{DiazModulus.no_algebraic_generalized_line} (hypothesis: Baker's theorem for two logarithms) \\
...
Thm~\ref{thm:saturation} & Proved, assuming Baker & \nolinkurl{DiazModulus.candidate_one_log_saturation} (hypothesis: Baker's theorem for two logarithms) \\
Cor.~\ref{cor:api} & Proved, assuming Baker & \nolinkurl{DiazModulus.candidate_one_log_saturation} (the same hypothesis) \\
