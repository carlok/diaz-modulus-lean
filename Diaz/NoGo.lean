/-
# Six exponentials cannot refute a candidate

Backup of `DiazModulus.sixExponentials_cannot_refute_candidate`
(`de25fa09-b094-4a91-a49a-1b0face79465`), Proved on Prove2Me.

Everything a candidate `u` certifies out of itself lies in the
three-dimensional space `V = span_Qbar {1, u, conj u}`: the modulus
hypothesis gives `conj u = |u|^2 / u`, and nothing else is forthcoming. The
statement has three parts. `V` sits inside `ℒ̃`. A *four* exponentials
template does fit inside `V`, which is why the strong four exponentials
conjecture settles the question. A *six* exponentials template does not fit
inside any three-dimensional space at all, independently of `u`: six
products of a 2-family and a 3-family, each independent, span at least
`2 + 3 - 1 = 4` dimensions.

The third clause is pure linear algebra with no transcendence input and no
candidacy hypothesis, which is what makes it a genuine obstruction rather
than a failed attempt. Its complement is
`candidate_multiplier_module` in `Diaz.Multipliers`: that one determines
what an extension of `V` would have to contain, and finds it closed under
nothing.
...
private theorem nogo_isAlgebraic_conj {z : ℂ} (h : IsAlgebraic ℚ z) : IsAlgebraic ℚ (conj z) := by
  obtain ⟨p, hp0, hpz⟩ := h
  refine ⟨p, hp0, ?_⟩
...
private theorem candidate_triple_independent {u : ℂ} (hu : IsCandidate u)
    (hHL : HermiteLindemannProp) : LinearIndependent (↥Qbar) ![(1 : ℂ), u, conj u] := by
  obtain ⟨hu0, hmod, hexp⟩ := hu
  have huQ : u ∉ Qbar := fun h => hHL u hu0 (mem_Qbar_iff.mp h) hexp
  have hcuQ : conj u ∉ Qbar := by
    intro h
    exact huQ (by
      have := mem_Qbar_iff.mpr (nogo_isAlgebraic_conj (mem_Qbar_iff.mp h))
      simpa using this)
...
theorem sixExponentials_cannot_refute_candidate (u : ℂ) :
    ...
    (IsCandidate u → HermiteLindemannProp →
