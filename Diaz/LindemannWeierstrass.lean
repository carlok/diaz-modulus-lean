/-
# Lindemann–Weierstrass, as much as Hermite–Lindemann needs

Ported from an unmerged Mathlib pull request by way of a Prove2Me submission. It
depends on Mathlib alone, which is the point of it being a separate module:
`Diaz.Axioms` imports it to prove Hermite–Lindemann, and everything in the library
imports `Diaz.Axioms`, so nothing here may import the library.
-/
import Mathlib
...
Lemmas used below that are not in this Mathlib revision. -/
...
/-! The symmetric-polynomial evaluation lemmas that used to be back-ported here
(`MvPolynomial.symmetricSubalgebra.aevalMultiset`, `sumPolynomial`,
`Polynomial.pow_smul_sum_map_aroots_aeval_mem_range_algebraMap`, ...) are now in
Mathlib (`Mathlib.RingTheory.MvPolynomial.Symmetric.Eval`), with identical
statements, and are used from there. -/
/-
Copyright (c) 2022 Yuyang Zhao. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Yuyang Zhao
...
/-! `Quot.liftFinsupp` and `Quotient.liftFinsupp` are now in Mathlib
(`Mathlib.Data.Finsupp.Quotient`), with identical statements. -/
