/-
Mirrored from Prove2Me: `DiazModulus.candidate_conj_product_rational`.

Ported mechanically from the accepted submission archived as
`archive/prove2me/DiazModulus.candidate_conj_product_rational__f181d390.lean`. Statement and proof are the platform's; only
...
noncomputable def candidate_conj_product_rational_cjQ : ℂ →ₐ[ℚ] ℂ :=
  (Complex.conjAe : ℂ ≃ₐ[ℝ] ℂ).toAlgHom.restrictScalars ℚ

theorem candidate_conj_product_rational_alg_conj {z : ℂ} (h : IsAlgebraic ℚ z) : IsAlgebraic ℚ (conj z) := by
  obtain ⟨p, hp0, hp⟩ := h
  refine ⟨p, hp0, ?_⟩
  have : Polynomial.aeval (candidate_conj_product_rational_cjQ z) p = candidate_conj_product_rational_cjQ (Polynomial.aeval z p) :=
    Polynomial.aeval_algHom_apply candidate_conj_product_rational_cjQ z p
  rw [show (conj z : ℂ) = candidate_conj_product_rational_cjQ z from rfl, this, hp, map_zero]
...
theorem candidate_conj_product_rational (p q : ℂ) (hp : IsCandidate p) (hq : IsCandidate q)
  ...
  have hgs := GelfondSchneider.gelfond_schneider (q / conj p) (conj p) hbalg hbq
    (by rw [Complex.exp_conj]; exact candidate_conj_product_rational_alg_conj hpe) hcp
