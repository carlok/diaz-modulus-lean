/-
Mirrored from Prove2Me: `Diaz.failure_rational_multiple_rigid`.

Ported mechanically from the accepted submission archived as
`archive/prove2me/Diaz.failure_rational_multiple_rigid__ebe064ae.lean`. Statement and proof are the platform's; only
...
private theorem failure_rational_multiple_rigid_polar2_isAlgebraic_ratCast (q : ℚ) : IsAlgebraic ℚ ((q : ℂ)) := by
...
theorem failure_rational_multiple_rigid {t₁ t₂ : ℝ} (ht₁ : t₁ ≠ 0)
  ...
  have hsq : IsAlgebraic ℚ ((t₁ ^ 2 : ℝ) : ℂ) := by
    ...
    rw [hrw]
    exact IsAlgebraic.mul (failure_rational_multiple_rigid_polar2_isAlgebraic_ratCast _) hdiff
  have ht₁alg : IsAlgebraic ℚ (((t₁ : ℝ)) : ℂ) := by
    refine IsAlgebraic.of_pow (n := 2) (by norm_num) ?_
    rwa [← Complex.ofReal_pow]
  have ht₁ne : ((t₁ : ℝ) : ℂ) ≠ 0 := by
    simpa using ht₁
  have := hermite_lindemann_holds _ ht₁ne ht₁alg
