/-
Mirrored from Prove2Me: `Diaz.candidate_indistinguishable_by_coeff`.

Ported mechanically from the accepted submission archived as
`archive/prove2me/Diaz.candidate_indistinguishable_by_coeff__311d5009.lean`. Statement and proof are the platform's; only
...
Source: an earlier unpublished note, Corollary 7
...
1. `Diaz.candidate_indistinguishable_by_coeff_coeff_transfer_iff` — the corollary as stated, both clauses, with
...
theorem Diaz.candidate_indistinguishable_by_coeff_coeff_transfer_iff {K : Subfield ℂ} {m n : Type*} [Fintype m] [Fintype n]
    (Φ : ℂ →+* ℂ) (hK : ∀ a ∈ K, Φ a = a)
...
`Diaz.candidate_indistinguishable`,
...
theorem candidate_indistinguishable_by_coeff_self_mem_hull (K : Subfield ℂ) (x : ℂ) : x ∈ hull K x :=
...
claimed for it here. -/
theorem candidate_indistinguishable_by_coeff
  ...
  have hmap : ∀ z ∈ hull Qbar u, Φ z ∈ hull Qbar t := by
    have hle : hull Qbar u ≤ Subfield.comap Φ (hull Qbar t) := by
      ...
      · show Φ x ∈ hull Qbar t
        rw [hΦu]
        exact Diaz.candidate_indistinguishable_by_coeff_self_mem_hull Qbar t
  ...
  · -- `t` lies on the same circle: apply `Φ` to `u * conj u = r ^ 2`.
    ...
    rw [map_mul, hconj u (Diaz.candidate_indistinguishable_by_coeff_self_mem_hull Qbar u), hΦu] at hru
    exact hru
  · intro p q M w v hM hw hv
    exact ⟨fun i j => hmap _ (hM i j),
      (Diaz.candidate_indistinguishable_by_coeff_coeff_transfer_iff Φ hfix M w v hw hv).2⟩
