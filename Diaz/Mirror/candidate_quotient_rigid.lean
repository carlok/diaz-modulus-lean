/-
Mirrored from Prove2Me: `DiazModulus.candidate_quotient_rigid`.

Ported mechanically from the accepted submission archived as
`archive/prove2me/DiazModulus.candidate_quotient_rigid__403a02f8.lean`. Statement and proof are the platform's; only
...
theorem candidate_quotient_rigid_exp_rat_mul_alg (q : ℚ) {w : ℂ} (hw : IsAlgebraic ℚ (Complex.exp w)) :
...
theorem candidate_quotient_rigid_ratAlg (q : ℚ) : IsAlgebraic ℚ (q : ℂ) := isAlgebraic_ratCast ℚ q

/-- `u ū = |u|²`. -/
theorem candidate_quotient_rigid_mul_conj_eq (u : ℂ) : u * conj u = ((‖u‖ : ℝ) : ℂ) ^ 2 := by
  rw [Complex.mul_conj, Complex.normSq_eq_norm_sq]; push_cast; ring

/-- For a candidate, `u ū` is algebraic. -/
theorem candidate_quotient_rigid_mul_conj_alg {u : ℂ} (hu : IsCandidate u) :
    IsAlgebraic ℚ (u * conj u) := by
  rw [candidate_quotient_rigid_mul_conj_eq]; exact hu.2.1.pow 2

/-- A candidate is off the real axis (otherwise `u = ±|u|` is algebraic; Hermite–Lindemann). -/
theorem candidate_quotient_rigid_im_ne_zero {u : ℂ} (hu : IsCandidate u) : u.im ≠ 0 := by
  obtain ⟨hu0, hn, he⟩ := hu
  intro h
  have hcu : conj u = u := Complex.conj_eq_iff_im.2 h
  have hsq : u ^ 2 = ((‖u‖ : ℝ) : ℂ) ^ 2 := by
    rw [← candidate_quotient_rigid_mul_conj_eq, hcu, sq]
...
theorem candidate_quotient_rigid_re_ne_zero {u : ℂ} (hu : IsCandidate u) : u.re ≠ 0 := by
  obtain ⟨hu0, hn, he⟩ := hu
  intro h
  have hcu : conj u = -u := by
    apply Complex.ext <;> simp [h]
  have hsq : u ^ 2 = -((‖u‖ : ℝ) : ℂ) ^ 2 := by
    rw [← candidate_quotient_rigid_mul_conj_eq, hcu]; ring
  have halg : IsAlgebraic ℚ u :=
    IsAlgebraic.of_pow two_pos (by rw [hsq]; exact (hn.pow 2).neg)
  exact hermite_lindemann_holds u hu0 halg he

theorem candidate_quotient_rigid_re_im_zero {x y : ℝ} (h : (x : ℂ) + (y : ℂ) * Complex.I = 0) : x = 0 ∧ y = 0 := by
...
theorem candidate_quotient_rigid_rel_re {u z : ℂ} {a b c : ℚ} (hu : u ≠ 0) (hne : ¬(a = 0 ∧ b = 0 ∧ c = 0))
    (hrel : (a : ℂ) * (u * conj u) + (b : ℂ) * z + (c : ℂ) * conj z = 0) (hY : z.im ≠ 0) :
    b ≠ 0 ∧ (a : ℂ) * (u * conj u) + 2 * (b : ℂ) * ((z.re : ℝ) : ℂ) = 0 := by
  have hρ : u * conj u = ((‖u‖ : ℝ) : ℂ) ^ 2 := candidate_quotient_rigid_mul_conj_eq u
  ...
  obtain ⟨hre, him⟩ := candidate_quotient_rigid_re_im_zero key
  have hbc : b = c := by
  ...
  · -- `v = a ū`
    ...
    have hsw : IsAlgebraic ℚ (Complex.exp (u * w / conj u)) := by
      have e : u * w / conj u = (a : ℂ) * (u * w / v) := by
        rw [ha]; field_simp
      rw [e]; exact candidate_quotient_rigid_exp_rat_mul_alg a hz
    obtain ⟨a', b, c', hne, hrel⟩ := conj_ratio_multiplier_relation u w
      (candidate_quotient_rigid_re_ne_zero hu) (candidate_quotient_rigid_im_ne_zero hu) hu.2.2 hw.2.2 hsw
    -- `Re(uw)` is algebraic
    have hX : IsAlgebraic ℚ (((u * w).re : ℝ) : ℂ) := by
      ...
      · obtain ⟨hb0, hXeq⟩ := candidate_quotient_rigid_rel_re hu0 hne hrel hY
        ...
        exact ((candidate_quotient_rigid_ratAlg a').mul (candidate_quotient_rigid_mul_conj_alg hu)).neg.mul (candidate_quotient_rigid_ratAlg (2 * b)).inv
    obtain ⟨r, hr⟩ := candidate_conj_product_rational u w hu hw hX
    exact Or.inr (Or.inr ⟨r, hr⟩)
  · -- generic case: `w = a v + b ū`
    push Not at h1 h2
    obtain ⟨a, b, hab⟩ := log_ratio_multipliers u v w hu0 hv0 hu.2.2 hv.2.2 hw.2.2
      c hc h1 h2 hz
    by_cases ha : a = 0
    · right; right
      refine ⟨b, ?_⟩
    ...
    have hX : IsAlgebraic ℚ (((u * v).re : ℝ) : ℂ) := by
      rw [hX', div_eq_mul_inv]
      exact (((candidate_quotient_rigid_mul_conj_alg hw).sub (((candidate_quotient_rigid_ratAlg a).pow 2).mul (candidate_quotient_rigid_mul_conj_alg hv))).sub
        (((candidate_quotient_rigid_ratAlg b).pow 2).mul (candidate_quotient_rigid_mul_conj_alg hu))).mul (candidate_quotient_rigid_ratAlg (2 * a * b)).inv
    obtain ⟨r, hr⟩ := candidate_conj_product_rational u v hu hv hX
    exact h2 r hr
