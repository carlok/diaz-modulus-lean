/-
Mirrored from Prove2Me: `DiazModulus.generic_quadratic_relation_is_norm`.

Ported mechanically from the accepted submission archived as
`archive/prove2me/DiazModulus.generic_quadratic_relation_is_norm__0f4f491e.lean`. Statement and proof are the platform's; only
...
noncomputable def Q (c0 c1 c2 c3 c4 c5 c6 c7 c8 : ↥Qbar) :
    MvPolynomial (Fin 2) ↥Qbar :=
  C c0 + C c1 * X 0 + C c2 * X 0 ^ 2 + C c3 * X 0 ^ 3 + C c4 * X 0 ^ 4
    + C c5 * X 0 * X 1 + C c6 * X 0 ^ 2 * X 1 + C c7 * X 0 ^ 3 * X 1 + C c8 * X 0 ^ 2 * X 1 ^ 2

theorem aeval_Q (c0 c1 c2 c3 c4 c5 c6 c7 c8 : ↥Qbar) (a b : ℂ) :
...
theorem generic_quadratic_relation_is_norm (u : ℂ) (hu : u ≠ 0)
  ...
  have hρQ : u * conj u ∈ Qbar := mem_Qbar_iff.2 hρ
  have hρ0 : u * conj u ≠ 0 := mul_ne_zero hu ((map_ne_zero _).2 hu)
  set w : ℂ := ((Real.pi : ℝ) : ℂ) * Complex.I with hw
  let r : ↥Qbar := ⟨u * conj u, hρQ⟩
  let q : Fin 4 → Fin 4 → ↥Qbar := fun k l => ⟨P k l, mem_Qbar_iff.2 (hP k l)⟩
  let Pu : MvPolynomial (Fin 2) ↥Qbar :=
  ...
  · exact mem_Qbar_iff.1
      (add_mem (mem_Qbar_iff.2 (hP 1 2)) (mem_Qbar_iff.2 (hP 2 1)))
  · intro x
