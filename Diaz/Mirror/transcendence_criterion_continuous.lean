...
`archive/prove2me/FourExp.transcendence_criterion_continuous__9e7becbe.lean`. Statement and proof are the platform's; only
...
theorem transcendence_criterion_continuous
  ...
  have hsmall : ((1 + ‖α‖) * ((d + δ : ℕ) : ℝ)) ^ (d + δ) * Real.exp u ^ δ * Real.exp (3 * u) ^ d
    ...
    linarith
  have hdvd : Q ∣ P N :=
    dvd_of_small_values (P N) Q hirr α (Real.exp u) (Real.exp (3 * u))
