/-
Mirrored from Prove2Me: `DiazModulus.log_ratio_multipliers`.

Ported mechanically from the accepted submission archived as
`archive/prove2me/DiazModulus.log_ratio_multipliers__2ed14ab9.lean`. Statement and proof are the platform's; only
...
noncomputable def log_ratio_multipliers_cjQ : ℂ →ₐ[ℚ] ℂ :=
  (Complex.conjAe : ℂ ≃ₐ[ℝ] ℂ).toAlgHom.restrictScalars ℚ

theorem log_ratio_multipliers_alg_conj {z : ℂ} (h : IsAlgebraic ℚ z) : IsAlgebraic ℚ (conj z) := by
  obtain ⟨p, hp0, hp⟩ := h
  refine ⟨p, hp0, ?_⟩
  have : Polynomial.aeval (log_ratio_multipliers_cjQ z) p = log_ratio_multipliers_cjQ (Polynomial.aeval z p) :=
    Polynomial.aeval_algHom_apply log_ratio_multipliers_cjQ z p
  rw [show (conj z : ℂ) = log_ratio_multipliers_cjQ z from rfl, this, hp, map_zero]

theorem log_ratio_multipliers_exp_conj_alg {w : ℂ} (hw : IsAlgebraic ℚ (Complex.exp w)) :
    IsAlgebraic ℚ (Complex.exp (conj w)) := by
  rw [Complex.exp_conj]; exact log_ratio_multipliers_alg_conj hw

theorem log_ratio_multipliers_exp_rat_mul_alg (q : ℚ) {w : ℂ} (hw : IsAlgebraic ℚ (Complex.exp w)) :
...
theorem log_ratio_multipliers (u v w : ℂ) (hu : u ≠ 0) (hv : v ≠ 0)
  ...
  have hall : ∀ i j, IsAlgebraic ℚ (Complex.exp (![(1 : ℂ), u / v] i * ![v, conj u, w] j)) := by
    ...
    · change IsAlgebraic ℚ (Complex.exp (1 * conj u)); rw [one_mul]; exact log_ratio_multipliers_exp_conj_alg heu
    ...
    · change IsAlgebraic ℚ (Complex.exp (u / v * conj u)); rw [e11]
      exact log_ratio_multipliers_exp_rat_mul_alg c (log_ratio_multipliers_exp_conj_alg hev)
