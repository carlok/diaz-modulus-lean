/-
Mirrored from Prove2Me: `Diaz.forced_plane_exhaustion`.
...
theorem forced_plane_exhaustion_aux_powers_indep {K : Subfield ℂ} {u : ℂ} (hT : Transcendental K u) (n : ℕ)
    (k : ℕ → ℂ) (hk : ∀ i, k i ∈ K)
...
theorem aux_binary_form {K : Subfield ℂ} {x y : ℂ} (hy : y ≠ 0)
  ...
  refine forced_plane_exhaustion_aux_powers_indep hxy (d + 1) c hc ?_
...
theorem forced_plane_exhaustion_aux_algebraMap_mk (K : Subfield ℂ) (a : ℂ) (h : a ∈ K) :
...
theorem forced_plane_exhaustion {K : Subfield ℂ} {u : ℂ} (hT : Transcendental K u)
