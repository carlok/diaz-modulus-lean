/-
Mirrored from Prove2Me: `GelfondSchneider.rho_house_le`.

Ported mechanically from the accepted submission archived as
`archive/prove2me/GelfondSchneider.rho_house_le__9046536c.lean`. Statement and proof are the platform's; only
...
lemma rho_house_le_house_natCast_eq (c : ℕ) : house (c : K) = c := by
...
lemma rho_house_le_house_lin_le (β : K) (a b q : ℕ) (ha : a < q) (hb : b < q) :
  ...
  calc house (((a + 1 : ℕ) : K) + ((b + 1 : ℕ) : K) * β)
    ...
    _ = ((a + 1 : ℕ) : ℝ) + ((b + 1 : ℕ) : ℝ) * house β := by
        rw [house_nat_mul, rho_house_le_house_natCast_eq]
    _ ≤ (q : ℝ) + (q : ℝ) * house β := by
        have := mul_le_mul_of_nonneg_right h2 h3
        linarith
    _ = (q : ℝ) * (1 + house β) := by ring

lemma rho_house_le_house_pow_le_max (y : K) (e N : ℕ) (h : e ≤ N) :
...
lemma rho_house_le_const_one_le {m : ℕ} (hm : 0 < m) {C₀ B A G : ℝ} (hC₀ : 1 ≤ C₀) (hB : 0 ≤ B)
    (hA : 1 ≤ A) (hG : 1 ≤ G) :
...
lemma rho_house_le_final_bound (m n q r : ℕ) (hm : 0 < m) (hn : 0 < n) (hq : q ^ 2 = 2 * m * n)
...
theorem rho_house_le (K : Type*) [Field K] [NumberField K] (α' β' γ' : K) (m : ℕ) (hm : 0 < m)
  ...
  refine ⟨(2 * (m : ℝ)) ^ 2 * C₀ * (1 + house β') *
    ...
    rho_house_le_const_one_le hm hC₀ (house_nonneg β') (le_max_left _ _) (le_max_left _ _), ?_⟩
  ...
  refine (house_double_sum_le _
    ...
    (rho_house_le_final_bound m n q r hm hn hq hnr C₀ (house β') _ _ hC₀ (house_nonneg β')
      (le_max_left _ _) (le_max_left _ _))
  intro a b
  exact house_term_le _ _ _ _ r _ _ _ _ _ _ (hη a b)
    (rho_house_le_house_lin_le β' a b q a.isLt b.isLt)
    (rho_house_le_house_pow_le_max α' _ _ (Nat.mul_le_mul (Nat.succ_le_of_lt a.isLt) hlm))
    (rho_house_le_house_pow_le_max γ' _ _ (Nat.mul_le_mul (Nat.succ_le_of_lt b.isLt) hlm))
