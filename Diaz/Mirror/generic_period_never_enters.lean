/-
Mirrored from Prove2Me: `DiazModulus.generic_period_never_enters`.

Ported mechanically from the accepted submission archived as
`archive/prove2me/DiazModulus.generic_period_never_enters__9164cebf.lean`. Statement and proof are the platform's; only
...
theorem generic_period_never_enters (u : ℂ) (hu : u ≠ 0)
  ...
  obtain ⟨c, hc_alg, hQ⟩ :=
    generic_quadratic_relation_is_norm u hu hρ hgen P hPalg hrel
  ...
  have hc0 : c ≠ 0 := by
    ...
    let A : Fin 2 → Fin 2 → Fin 4 → ↥Qbar :=
      fun i j k => ⟨C i j k, mem_Qbar_iff.2 (hC i j k)⟩
    have hA : ∀ i j k, ((A i j k : ↥Qbar) : ℂ) = C i j k := fun _ _ _ => rfl
    ...
    · have hr' : ∀ j k, (p : ℂ) * C 0 j k + (q : ℂ) * C 1 j k = 0 := by
      ...
      obtain ⟨hp, hq⟩ := hrow p q (mem_Qbar_iff.1 p.2)
        (mem_Qbar_iff.1 q.2) hM'
    ...
    · have hr' : ∀ i k, (p : ℂ) * C i 0 k + (q : ℂ) * C i 1 k = 0 := by
      ...
      obtain ⟨hp, hq⟩ := hcol p q (mem_Qbar_iff.1 p.2)
        (mem_Qbar_iff.1 q.2) hM'
      exact hpq ⟨Subtype.ext hp, Subtype.ext hq⟩
  ...
  have h23 := hb 2 3
  have h33 := hb 3 3
  simp only [hPdef, Pi.single_apply, Fin.reduceEq, ite_true, ite_false, mul_one, mul_zero, add_zero,
