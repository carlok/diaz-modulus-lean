/-
Mirrored from Prove2Me: `DiazModulus.candidate_exp_angularTriple_transcendental`.

Ported mechanically from the accepted submission archived as
`archive/prove2me/DiazModulus.candidate_exp_angularTriple_transcendental__212c5c15.lean`. Statement and proof are the platform's; only
...
private theorem candidate_exp_angularTriple_transcendental_isAlgebraic_conj {z : ℂ} (h : IsAlgebraic ℚ z) :
...
private theorem candidate_exp_angularTriple_transcendental_core (hHL : HermiteLindemann)
  ...
  have hcQ : u * conj u ∈ Qbar := by
    rw [hcc]; exact pow_mem (mem_Qbar_iff.mpr hmod) 2
  ...
  have httr : ¬ IsAlgebraic ℚ t := by
    ...
    rw [hu2]
    exact mem_Qbar_iff.mp
      (mul_mem hcQ (mem_Qbar_iff.mpr hta))
  ...
  have hxli : LinearIndependent ℚ ![u, conj u] := by
    ...
    · exfalso
      ...
      rw [hu2]
      refine mem_Qbar_iff.mp
        (mul_mem (inv_mem ?_) (mul_mem (neg_mem ?_) hcQ))
      · exact SubfieldClass.ratCast_mem Qbar a
      · exact SubfieldClass.ratCast_mem Qbar b
  ...
  have hconj : IsAlgebraic ℚ (Complex.exp ((conj u) ^ 2 / u)) := by
    ...
    exact candidate_exp_angularTriple_transcendental_isAlgebraic_conj hcon
  have hcexp : IsAlgebraic ℚ (Complex.exp (conj u)) := by
    rw [Complex.exp_conj]; exact candidate_exp_angularTriple_transcendental_isAlgebraic_conj hexp
...
theorem candidate_exp_angularTriple_transcendental {u : ℂ} (h : IsCandidate u) :
    Transcendental ℚ (Complex.exp (u ^ 2 / conj u)) :=
  candidate_exp_angularTriple_transcendental_core hermite_lindemann_holds six_exponentials h
