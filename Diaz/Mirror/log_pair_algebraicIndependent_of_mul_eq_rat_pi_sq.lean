/-
Mirrored from Prove2Me: `DiazModulus.log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq`.

Ported mechanically from the accepted submission archived as
`archive/prove2me/DiazModulus.log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq__9edec8e3.lean`. Statement and proof are the platform's; only
...
noncomputable def log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E (x : ℂ) : Subalgebra ℚ ℂ :=
  (Subalgebra.algebraicClosure ↥(Algebra.adjoin ℚ ({x} : Set ℂ)) ℂ).restrictScalars ℚ

theorem log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_mem_E_iff {x z : ℂ} : z ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E x ↔ IsAlgebraic ↥(Algebra.adjoin ℚ ({x} : Set ℂ)) z :=
  Iff.rfl

theorem log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_mem_E_of_alg {x z : ℂ} (h : IsAlgebraic ℚ z) : z ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E x :=
  h.extendScalars (algebraMap ℚ ↥(Algebra.adjoin ℚ ({x} : Set ℂ))).injective

theorem rat_mem_E (x : ℂ) (q : ℚ) : (q : ℂ) ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E x :=
  log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_mem_E_of_alg (isAlgebraic_algebraMap q)

theorem log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_self_mem_E (x : ℂ) : x ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E x := by
  rw [log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_mem_E_iff]
  have h : x = algebraMap ↥(Algebra.adjoin ℚ ({x} : Set ℂ)) ℂ
      ⟨x, Algebra.subset_adjoin rfl⟩ := rfl
  rw [h]
  exact isAlgebraic_algebraMap _

theorem log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_mem_E_of_mul {x a z : ℂ} (ha0 : a ≠ 0) (ha : a ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E x) (h : a * z ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E x) :
    z ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E x := by
  rw [log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_mem_E_iff] at h ⊢
  exact IsAlgebraic.of_mul (mem_nonZeroDivisors_of_ne_zero ha0) (log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_mem_E_iff.1 ha) h

theorem log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_mem_E_of_sq {x z : ℂ} (h : z ^ 2 ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E x) : z ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E x :=
  (log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_mem_E_iff.1 h).of_pow (by norm_num)
...
theorem mem_E_of_not_indep {a b : ℂ} (ha : Transcendental ℚ a)
    (h : ¬ AlgebraicIndependent ℚ ![a, b]) : b ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E a := by
  by_contra hb
  exact h (algIndep_pair ha (fun hb' => hb (log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_mem_E_iff.2 hb')))
...
transcendental. -/
theorem log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_transc_of_exp {z : ℂ} (hz : z ≠ 0) (he : IsAlgebraic ℚ (Complex.exp z)) :
...
theorem key (l₁ l₂ : ℂ)
    ...
    (hT : 2 * ((Real.pi : ℝ) : ℂ) * Complex.I ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E l₁) (hl₂ : l₂ ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E l₁) : False := by
  have hT0 := T_ne_zero
  ...
  have htr := Transcendence.trdeg_adjoin_le_one_of_isAlgebraic_adjoin (K := ℚ) l₁
    ({l₁, ((-c / 4 : ℚ) : ℂ) * (2 * ((Real.pi : ℝ) : ℂ) * Complex.I),
      ...
      · exact log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_self_mem_E l₁
  ...
  have hexpT : IsAlgebraic ℚ (Complex.exp (2 * ((Real.pi : ℝ) : ℂ) * Complex.I)) := by
    rw [exp_T]
    exact isAlgebraic_one
  rcases four_exponentials_trdeg_one l₁
...
theorem log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq (l₁ l₂ : ℂ)
  ...
  have hl₁0 : l₁ ≠ 0 := by simpa using hroot 0
  have hcC : (c : ℂ) ≠ 0 := by exact_mod_cast hc
  have htr₁ : Transcendental ℚ l₁ := log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_transc_of_exp hl₁0 h₁
  constructor
  · -- if `![l₁, l₂]` is dependent, `l₂` and then `T` are algebraic over `ℚ[l₁]`
    by_contra hdep
    have hl₂ : l₂ ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E l₁ := mem_E_of_not_indep htr₁ hdep
    have hT : 2 * ((Real.pi : ℝ) : ℂ) * Complex.I ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E l₁ := by
      apply log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_mem_E_of_sq
      have hsq : (2 * ((Real.pi : ℝ) : ℂ) * Complex.I) ^ 2 =
        ...
        ring
      rw [hsq]
      exact mul_mem (rat_mem_E _ _) (mul_mem (log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_self_mem_E l₁) hl₂)
    exact key l₁ l₂ h₁ h₂ hroot c hc hprod hT hl₂
  · -- if `![l₁, T]` is dependent, `T` and then `l₂` are algebraic over `ℚ[l₁]`
    by_contra hdep
    have hT : 2 * ((Real.pi : ℝ) : ℂ) * Complex.I ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E l₁ := mem_E_of_not_indep htr₁ hdep
    have hl₂ : l₂ ∈ log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_E l₁ := by
      apply log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_mem_E_of_mul hl₁0 (log_pair_algebraicIndependent_of_mul_eq_rat_pi_sq_self_mem_E l₁)
      have hmul : l₁ * l₂ =
