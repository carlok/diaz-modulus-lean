...
`archive/prove2me/Schanuel.gelfond_schneider__129e4463.lean`. Statement and proof are the platform's; only
...
lemma gs_analyticOrderAt_iterated_deriv {𝕜 : Type*} {E : Type*} [NontriviallyNormedField 𝕜]
  [NormedAddCommGroup E] [NormedSpace 𝕜 E] [CompleteSpace E] {f : 𝕜 → E} {z₀ : 𝕜}
...
lemma gs_analyticOrderAt_eq_nat_iff_iteratedDeriv_eq_zero {z₀ : ℂ} (n : ℕ) :
  ...
  | succ n IH =>
    ...
    · refine fun ho ↦ ⟨fun k hk ↦ (analyticOrderAt_ne_zero.mp ?_).2, ?_⟩
      · grind only [(gs_analyticOrderAt_iterated_deriv (f:=f) hf k (n := (n + 1))
          ho.symm (by grind) hk.le), Nat.cast_ne_zero]
      · have := gs_analyticOrderAt_iterated_deriv (f := f) hf (n + 1) (n := n + 1)
