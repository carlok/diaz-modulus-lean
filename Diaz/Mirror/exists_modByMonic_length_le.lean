/-
Mirrored from Prove2Me: `Transcendence.exists_modByMonic_length_le`.

Ported mechanically from the accepted submission archived as
`archive/prove2me/Transcendence.exists_modByMonic_length_le__c575b504.lean`. Statement and proof are the platform's; only
...
coefficient of `R_k`, `exists_modByMonic_length_le_len R_{k+1} ≤ exists_modByMonic_length_le_len R_k + exists_modByMonic_length_le_len R_k · exists_modByMonic_length_le_len Q`, so `exists_modByMonic_length_le_len R_k ≤ (1 + exists_modByMonic_length_le_len Q)ᵏ`,
...
`P %ₘ Q = ∑ₖ Pₖ R_k`, and the bounds follow with `C = 1 + exists_modByMonic_length_le_len Q + D`.
...
noncomputable def exists_modByMonic_length_le_len (P : Polynomial (Polynomial ℤ)) : ℕ :=
  ∑ k ∈ P.support, len1 (P.coeff k)

theorem len_mul_le (P R : Polynomial (Polynomial ℤ)) : exists_modByMonic_length_le_len (P * R) ≤ exists_modByMonic_length_le_len P * exists_modByMonic_length_le_len R :=
  Transcendence.length_mul_le P R

theorem len_sum_le {ι : Type*} (s : Finset ι) (f : ι → Polynomial (Polynomial ℤ)) :
    exists_modByMonic_length_le_len (∑ i ∈ s, f i) ≤ ∑ i ∈ s, exists_modByMonic_length_le_len (f i) :=
  Transcendence.length_sum_le s f

theorem len1_zero : len1 0 = 0 := by
  simp only [len1, support_zero, Finset.sum_empty]

theorem len_zero : exists_modByMonic_length_le_len 0 = 0 := by
  simp only [exists_modByMonic_length_le_len, support_zero, Finset.sum_empty]
...
theorem len_eq_sum (P : Polynomial (Polynomial ℤ)) {s : Finset ℕ} (hs : P.support ⊆ s) :
    exists_modByMonic_length_le_len P = ∑ k ∈ s, len1 (P.coeff k) := by
...
theorem len1_coeff_le (P : Polynomial (Polynomial ℤ)) (k : ℕ) : len1 (P.coeff k) ≤ exists_modByMonic_length_le_len P := by
...
theorem len_C (p : Polynomial ℤ) : exists_modByMonic_length_le_len (C p) = len1 p := by
  rw [len_eq_sum (C p) (support_C_subset p), Finset.sum_singleton, coeff_C_zero]

theorem len_one : exists_modByMonic_length_le_len 1 = 1 := by
  rw [← C_1, len_C, len1_one]

theorem len_X : exists_modByMonic_length_le_len (X : Polynomial (Polynomial ℤ)) = 1 := by
  have hs : (X : Polynomial (Polynomial ℤ)).support ⊆ {1} := fun i hi => by
    rwa [support_X] at hi
  rw [len_eq_sum X hs, Finset.sum_singleton, coeff_X_one, len1_one]

theorem len_add_le (A B : Polynomial (Polynomial ℤ)) : exists_modByMonic_length_le_len (A + B) ≤ exists_modByMonic_length_le_len A + exists_modByMonic_length_le_len B := by
  have h := len_sum_le Finset.univ (fun b : Bool => cond b A B)
  rw [Fintype.sum_bool, Fintype.sum_bool] at h
  exact h

theorem len_neg (A : Polynomial (Polynomial ℤ)) : exists_modByMonic_length_le_len (-A) = exists_modByMonic_length_le_len A := by
  unfold exists_modByMonic_length_le_len len1
  rw [support_neg]
...
theorem len_sub_le (A B : Polynomial (Polynomial ℤ)) : exists_modByMonic_length_le_len (A - B) ≤ exists_modByMonic_length_le_len A + exists_modByMonic_length_le_len B := by
...
/-- `exists_modByMonic_length_le_len (Yᵏ %ₘ Q) ≤ (1 + exists_modByMonic_length_le_len Q)ᵏ`, and the coefficients of `Yᵏ %ₘ Q` have `X`-degree at
most `k D`. -/
theorem X_pow_modByMonic_le {Q : Polynomial (Polynomial ℤ)} (hQ : Q.Monic)
    (hd : 1 ≤ Q.natDegree) {D : ℕ} (hD : ∀ j, (Q.coeff j).natDegree ≤ D) :
    ∀ k : ℕ, exists_modByMonic_length_le_len (X ^ k %ₘ Q) ≤ (1 + exists_modByMonic_length_le_len Q) ^ k ∧
  ...
  | k + 1 => by
    ...
    refine ⟨?_, ?_⟩
    · calc exists_modByMonic_length_le_len (X * (X ^ k %ₘ Q) - C ((X ^ k %ₘ Q).coeff (Q.natDegree - 1)) * Q)
          ≤ exists_modByMonic_length_le_len (X * (X ^ k %ₘ Q)) + exists_modByMonic_length_le_len (C ((X ^ k %ₘ Q).coeff (Q.natDegree - 1)) * Q) :=
            len_sub_le _ _
        _ ≤ exists_modByMonic_length_le_len (X ^ k %ₘ Q) + exists_modByMonic_length_le_len (X ^ k %ₘ Q) * exists_modByMonic_length_le_len Q := by
        ...
        _ = exists_modByMonic_length_le_len (X ^ k %ₘ Q) * (1 + exists_modByMonic_length_le_len Q) := by ring
        _ ≤ (1 + exists_modByMonic_length_le_len Q) ^ k * (1 + exists_modByMonic_length_le_len Q) := Nat.mul_le_mul_right _ ih1
        _ = (1 + exists_modByMonic_length_le_len Q) ^ (k + 1) := (pow_succ _ _).symm
...
theorem main_monic {Q : Polynomial (Polynomial ℤ)} (hQ : Q.Monic) (hd : 1 ≤ Q.natDegree)
    (P : Polynomial (Polynomial ℤ)) (b e n : ℕ) (hb : exists_modByMonic_length_le_len P ≤ b)
    (he : ∀ k, (P.coeff k).natDegree ≤ e) (hn : P.natDegree ≤ n) :
    exists_modByMonic_length_le_len (P %ₘ Q) ≤ (1 + exists_modByMonic_length_le_len Q + degXQ Q) ^ n * b ∧
      ∀ k, ((P %ₘ Q).coeff k).natDegree ≤ e + (1 + exists_modByMonic_length_le_len Q + degXQ Q) * n := by
  have hR := X_pow_modByMonic_le hQ hd (natDegree_coeff_le_degXQ Q)
  have hK : 1 ≤ 1 + exists_modByMonic_length_le_len Q + degXQ Q := by omega
  rw [modByMonic_eq_sum Q P]
  refine ⟨?_, fun j => ?_⟩
  · calc exists_modByMonic_length_le_len (∑ k ∈ Finset.range (P.natDegree + 1), C (P.coeff k) * (X ^ k %ₘ Q))
        ≤ ∑ k ∈ Finset.range (P.natDegree + 1), exists_modByMonic_length_le_len (C (P.coeff k) * (X ^ k %ₘ Q)) :=
          len_sum_le _ _
      _ ≤ ∑ k ∈ Finset.range (P.natDegree + 1),
            len1 (P.coeff k) * (1 + exists_modByMonic_length_le_len Q + degXQ Q) ^ n := by
          ...
          calc (1 + exists_modByMonic_length_le_len Q) ^ k ≤ (1 + exists_modByMonic_length_le_len Q + degXQ Q) ^ k :=
                Nat.pow_le_pow_left (Nat.le_add_right _ _) k
            _ ≤ (1 + exists_modByMonic_length_le_len Q + degXQ Q) ^ n := Nat.pow_le_pow_right hK hkn
      _ = exists_modByMonic_length_le_len P * (1 + exists_modByMonic_length_le_len Q + degXQ Q) ^ n := by
          rw [← Finset.sum_mul, len_eq_sum P supp_subset_range_natDegree_succ]
      _ ≤ b * (1 + exists_modByMonic_length_le_len Q + degXQ Q) ^ n := Nat.mul_le_mul_right _ hb
      _ = (1 + exists_modByMonic_length_le_len Q + degXQ Q) ^ n * b := Nat.mul_comm _ _
  · rw [finsetSum_coeff]
    ...
    calc k * degXQ Q ≤ n * (1 + exists_modByMonic_length_le_len Q + degXQ Q) := Nat.mul_le_mul hkn (Nat.le_add_left _ _)
      _ = (1 + exists_modByMonic_length_le_len Q + degXQ Q) * n := Nat.mul_comm _ _

theorem exists_modByMonic_length_le_main (Q : Polynomial (Polynomial ℤ)) :
    ∃ C : ℕ, ∀ (P : Polynomial (Polynomial ℤ)) (b e n : ℕ),
      exists_modByMonic_length_le_len P ≤ b → (∀ k, (P.coeff k).natDegree ≤ e) → P.natDegree ≤ n →
      exists_modByMonic_length_le_len (P %ₘ Q) ≤ C ^ n * b ∧ ∀ k, ((P %ₘ Q).coeff k).natDegree ≤ e + C * n := by
  by_cases hQ : Q.Monic
  · by_cases hd : Q.natDegree = 0
    · refine ⟨1, fun P b e n _ _ _ => ?_⟩
      ...
      exact Nat.zero_le _
    · exact ⟨1 + exists_modByMonic_length_le_len Q + degXQ Q, fun P b e n hb he hn =>
...
theorem exists_modByMonic_length_le (Q : Polynomial (Polynomial ℤ)) :
  ...
  exact S7W3_exists_modByMonic_length_le.exists_modByMonic_length_le_main Q
