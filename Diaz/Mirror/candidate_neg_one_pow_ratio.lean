/-
Mirrored from Prove2Me: `DiazModulus.candidate_neg_one_pow_ratio`.

Ported mechanically from the accepted submission archived as
`archive/prove2me/DiazModulus.candidate_neg_one_pow_ratio__74c2ce0a.lean`. Statement and proof are the platform's; only
...
noncomputable def candidate_neg_one_pow_ratio_cjQ : ℂ →ₐ[ℚ] ℂ :=
  (Complex.conjAe : ℂ ≃ₐ[ℝ] ℂ).toAlgHom.restrictScalars ℚ

theorem candidate_neg_one_pow_ratio_alg_conj {z : ℂ} (h : IsAlgebraic ℚ z) : IsAlgebraic ℚ (conj z) := by
  obtain ⟨p, hp0, hp⟩ := h
  refine ⟨p, hp0, ?_⟩
  have : Polynomial.aeval (candidate_neg_one_pow_ratio_cjQ z) p = candidate_neg_one_pow_ratio_cjQ (Polynomial.aeval z p) :=
    Polynomial.aeval_algHom_apply candidate_neg_one_pow_ratio_cjQ z p
  rw [show (conj z : ℂ) = candidate_neg_one_pow_ratio_cjQ z from rfl, this, hp, map_zero]

/-- `ℒ` is stable under conjugation. -/
theorem candidate_neg_one_pow_ratio_exp_conj_alg {w : ℂ} (hw : IsAlgebraic ℚ (Complex.exp w)) :
    IsAlgebraic ℚ (Complex.exp (conj w)) := by
  rw [Complex.exp_conj]; exact candidate_neg_one_pow_ratio_alg_conj hw

theorem candidate_neg_one_pow_ratio_alg_I : IsAlgebraic ℚ Complex.I := by
...
theorem mul_conj_alg {u : ℂ} (hu : IsCandidate u) :
...
theorem im_ne_zero {u : ℂ} (hu : IsCandidate u) : u.im ≠ 0 := by
  obtain ⟨hu0, hn, he⟩ := hu
  intro h
  ...
  exact hermite_lindemann_holds u hu0 halg he

/-- A candidate is off the imaginary axis (otherwise `u = ±i|u|` is algebraic). -/
theorem re_ne_zero {u : ℂ} (hu : IsCandidate u) : u.re ≠ 0 := by
  obtain ⟨hu0, hn, he⟩ := hu
  intro h
...
theorem candidate_neg_one_pow_ratio_trdeg_le_one_of_adjoin_singleton
...
noncomputable def candidate_neg_one_pow_ratio_E (x : ℂ) : Subalgebra ℚ ℂ :=
  (Subalgebra.algebraicClosure ↥(Algebra.adjoin ℚ ({x} : Set ℂ)) ℂ).restrictScalars ℚ

theorem candidate_neg_one_pow_ratio_mem_E_iff {x z : ℂ} : z ∈ candidate_neg_one_pow_ratio_E x ↔ IsAlgebraic ↥(Algebra.adjoin ℚ ({x} : Set ℂ)) z :=
  Iff.rfl

theorem candidate_neg_one_pow_ratio_mem_E_of_alg {x z : ℂ} (h : IsAlgebraic ℚ z) : z ∈ candidate_neg_one_pow_ratio_E x :=
  h.extendScalars (algebraMap ℚ ↥(Algebra.adjoin ℚ ({x} : Set ℂ))).injective

theorem candidate_neg_one_pow_ratio_self_mem_E (x : ℂ) : x ∈ candidate_neg_one_pow_ratio_E x := by
  rw [candidate_neg_one_pow_ratio_mem_E_iff]
  have h : x = algebraMap ↥(Algebra.adjoin ℚ ({x} : Set ℂ)) ℂ
      ⟨x, Algebra.subset_adjoin rfl⟩ := rfl
  rw [h]
  exact isAlgebraic_algebraMap _

theorem inv_mem_E {x z : ℂ} (h : z ∈ candidate_neg_one_pow_ratio_E x) : z⁻¹ ∈ candidate_neg_one_pow_ratio_E x := by
  rw [candidate_neg_one_pow_ratio_mem_E_iff] at h ⊢
  exact h.inv

theorem candidate_neg_one_pow_ratio_mem_E_of_mul {x z : ℂ} (hx : x ≠ 0) (h : x * z ∈ candidate_neg_one_pow_ratio_E x) : z ∈ candidate_neg_one_pow_ratio_E x := by
  rw [candidate_neg_one_pow_ratio_mem_E_iff] at h ⊢
  exact IsAlgebraic.of_mul (mem_nonZeroDivisors_of_ne_zero hx) (candidate_neg_one_pow_ratio_mem_E_iff.1 (candidate_neg_one_pow_ratio_self_mem_E x)) h

theorem mem_E_of_sq {x z : ℂ} (h : z ^ 2 ∈ candidate_neg_one_pow_ratio_E x) : z ∈ candidate_neg_one_pow_ratio_E x := by
  rw [candidate_neg_one_pow_ratio_mem_E_iff] at h ⊢
...
theorem candidate_neg_one_pow_ratio (u : ℂ) (hu : IsCandidate u) :
  ...
  obtain ⟨a, b, c, hne, hrel⟩ :=
    conj_ratio_multiplier_relation u w₀ hre him hu.2.2 hew₀ hsw
  ...
  -- Step 2: everything lies in `candidate_neg_one_pow_ratio_E w₀`, the numbers algebraic over `ℚ[πi]`
  have hρE : u * conj u ∈ candidate_neg_one_pow_ratio_E w₀ := candidate_neg_one_pow_ratio_mem_E_of_alg (mul_conj_alg hu)
  have hIE : Complex.I ∈ candidate_neg_one_pow_ratio_E w₀ := candidate_neg_one_pow_ratio_mem_E_of_alg candidate_neg_one_pow_ratio_alg_I
  have hθE : ((u.im : ℝ) : ℂ) ∈ candidate_neg_one_pow_ratio_E w₀ := by
    refine candidate_neg_one_pow_ratio_mem_E_of_mul hw₀0 ?_
    rw [hθw, div_eq_mul_inv]
    exact mul_mem (mul_mem (mul_mem (candidate_neg_one_pow_ratio_mem_E_of_alg (ratAlg a)) hρE) hIE)
      (candidate_neg_one_pow_ratio_mem_E_of_alg (ratAlg (2 * b)).inv)
  have htE : ((u.re : ℝ) : ℂ) ∈ candidate_neg_one_pow_ratio_E w₀ := by
  ...
  have huE : u ∈ candidate_neg_one_pow_ratio_E w₀ := by
    rw [← Complex.re_add_im u]
    exact add_mem htE (mul_mem hθE hIE)
  have hcuE : conj u ∈ candidate_neg_one_pow_ratio_E w₀ := by
    have e : conj u = ((u.re : ℝ) : ℂ) - ((u.im : ℝ) : ℂ) * Complex.I := by
      apply Complex.ext <;> simp
    rw [e]
    exact sub_mem htE (mul_mem hθE hIE)
  have h11E : w₀ * u / conj u ∈ candidate_neg_one_pow_ratio_E w₀ := by
    rw [div_eq_mul_inv]
    exact mul_mem (mul_mem (candidate_neg_one_pow_ratio_self_mem_E w₀) huE) (inv_mem_E hcuE)
  ...
  have htr : Algebra.trdeg ℚ
    ...
    refine candidate_neg_one_pow_ratio_trdeg_le_one_of_adjoin_singleton hxB ?_
    have hle : Algebra.adjoin ℚ ({w₀ * u / conj u, w₀, u, conj u} : Set ℂ) ≤ candidate_neg_one_pow_ratio_E w₀ := by
      refine Algebra.adjoin_le ?_
      rintro z hz
      simp only [Set.mem_insert_iff, Set.mem_singleton_iff] at hz
      rcases hz with h | h | h | h <;> rw [h, SetLike.mem_coe]
      exacts [h11E, candidate_neg_one_pow_ratio_self_mem_E w₀, huE, hcuE]
    intro y hy
    exact candidate_neg_one_pow_ratio_mem_E_iff.1 (hle hy)
  ...
  rcases four_exponentials_trdeg_one (w₀ * u / conj u) w₀ u (conj u)
      hs hew₀ hu.2.2 (candidate_neg_one_pow_ratio_exp_conj_alg hu.2.2) h110 hw₀0 hu0 hcu hdet htr with
