/-
Mirrored from Prove2Me: `DiazModulus.aligned_norm_free_no_quadratic_relation`.

Ported mechanically from the accepted submission archived as
`archive/prove2me/DiazModulus.aligned_norm_free_no_quadratic_relation__6aa4f67f.lean`. Statement and proof are the platform's; only
...
theorem aligned_norm_free_no_quadratic_relation_alg_rat (q : ℚ) : IsAlgebraic ℚ (((q : ℝ)) : ℂ) := by
...
theorem pi_sq_coeffs {c₀ c₁ c₂ : ℝ} (h₀ : IsAlgebraic ℚ (c₀ : ℂ))
  ...
  have hT : Transcendental ℚ (((Real.pi : ℝ) : ℂ) ^ 2) :=
    pi_transcendental.pow (by norm_num)
...
theorem six_coeffs_eq_zero (t y β ρ : ℝ) (r a b c d e f : ℚ) (ht : t ≠ 0) (hβ0 : β ≠ 0)
  ...
  obtain ⟨-, hI1, hI0⟩ := pi_sq_coeffs (c₂ := ((0 : ℚ) : ℝ))
    ...
    (by rw [Complex.ofReal_mul]; exact (aligned_norm_free_no_quadratic_relation_alg_rat _).mul hβ) (aligned_norm_free_no_quadratic_relation_alg_rat _) (aligned_norm_free_no_quadratic_relation_alg_rat _)
  ...
  obtain ⟨hR2, hR1, hR0⟩ := pi_sq_coeffs
    ...
    (c₀ := ((-2 * (a + b) : ℚ) : ℝ) * β ^ 2)
    (by rw [Complex.ofReal_mul, Complex.ofReal_pow]; exact (aligned_norm_free_no_quadratic_relation_alg_rat _).mul (hβ.pow 2))
    (by
      rw [Complex.ofReal_add, Complex.ofReal_mul, Complex.ofReal_mul]
      exact ((aligned_norm_free_no_quadratic_relation_alg_rat _).mul hρ).add ((aligned_norm_free_no_quadratic_relation_alg_rat _).mul hβ))
    (aligned_norm_free_no_quadratic_relation_alg_rat _)
    (by
      push_cast
