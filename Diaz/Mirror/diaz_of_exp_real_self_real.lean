/-
Mirrored from Prove2Me: `DiazModulus.diaz_of_exp_real_self_real`.

Ported mechanically from the accepted submission archived as
`archive/prove2me/DiazModulus.diaz_of_exp_real_self_real__63f2aa38.lean`. Statement and proof are the platform's; only
imports, namespaces and the theorem's name were rewritten.
-/
import Mathlib
import Diaz.Mirror.diaz_on_axes_of_hermite_lindemann
import Diaz.HermiteLindemann

namespace Diaz

open Complex ComplexConjugate

theorem diaz_of_exp_real_self_real :
    ∀ u : ℂ, u ≠ 0 → IsAlgebraic ℚ ((‖u‖ : ℝ) : ℂ) → (Complex.exp u).im = 0 →
      u.im = 0 → Transcendental ℚ (Complex.exp u) := by
  intro u hu hmod _ him
  exact diaz_on_axes_of_hermite_lindemann hermite_lindemann_holds u hu (Or.inl him) hmod

end Diaz
