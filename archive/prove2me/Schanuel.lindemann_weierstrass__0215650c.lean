...
This is `linearIndependent_exp'` above, the Lindemann–Weierstrass theorem in Baker's form, with
`IsAlgebraic ℚ` read as `IsIntegral ℚ` (the two agree over a field) and the conclusion restated:
a vanishing combination has all coefficients zero, so if one coefficient is non-zero the
combination does not vanish. The development above is a port of an unmerged Mathlib pull request
on Lindemann–Weierstrass; its authors are credited in the header. -/
theorem solution (n : ℕ) (a b : Fin n → ℂ)
    (ha : ∀ i, IsAlgebraic ℚ (a i)) (hainj : Function.Injective a)
    (hb : ∀ i, IsAlgebraic ℚ (b i)) (hb0 : ∃ i, b i ≠ 0) :
    ∑ i, b i * Complex.exp (a i) ≠ 0 := by
  intro h
  obtain ⟨i, hi⟩ := hb0
  have hv := linearIndependent_exp' a (fun j => isAlgebraic_iff_isIntegral.mp (ha j)) hainj
    b (fun j => isAlgebraic_iff_isIntegral.mp (hb j)) h
  exact hi (congrFun hv i)
