# Siegel's lemma builds the auxiliary function, with algebraic exponentials (Waldschmidt 1973, Lemma 4)

- **Node:** `FourExp.auxiliary_function_alg`
- **Status:** Open
- **Theorem id:** `88de0071-cb97-40ae-90e2-33e7eb593322`
...
**The auxiliary function.** Let $x_1, x_2, y_1, y_2 \in \mathbb{C}$, suppose every $e^{x_iy_j}$ is algebraic, and suppose $\omega$ is transcendental; $\omega_1$ is a root of $Q \in \mathbb{Z}[X][Y]$, monic in $Y$ of degree $d \ge 1$ and minimal, in the sense that no non-zero $A \in \mathbb{Z}[X][Y]$ with $\deg_Y A < d$ vanishes at $(\omega, \omega_1)$; and $D, E_i, G_j, H_{ij} \in \mathbb{Z}[X][Y]$ with $D(\omega,\omega_1) \ne 0$, $x_i D = E_i$, $y_j D = G_j$ and $e^{x_iy_j} D = H_{ij}$ at $(\omega, \omega_1)$. Then there is $\kappa > 0$ such that for every large $N$, with $S, t_1, t_2$ as below, there are $M \le \kappa S$ and integers $q(i,j,k',\mu,\nu)$, for $i<S$, $j,k'<2N$, $\mu<M$, $\nu<d$, with the following properties.
...
**Why this node exists.** It restates `FourExp.auxiliary_function` with the extra hypothesis that every $e^{x_iy_j}$ is algebraic. The function involves $e^{(jx_1+k'x_2)(ay_1+by_2)}$, a product of powers of the $e^{x_iy_j}$ with exponents up to $2N\lfloor N\sqrt{\log N}\rfloor$. When these numbers are algebraic, every power lies in a fixed finite-dimensional $\mathbb{Q}$-space, so its degree in $\omega$ stays bounded and only its height grows. Without the hypothesis the $\omega$-degree grows like $N^2\sqrt{\log N}$, and the unknowns no longer outnumber the equations. The earlier node is left as it is.
