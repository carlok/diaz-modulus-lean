...
/-! ## From Lindemann–Weierstrass to the rank-one matrix over `Q̄` -/
...
/-- If `u * a + r * b = 0` with `a, b` in `K`, not both zero, and `r ≠ 0` in `K`,
then `u ∈ K`. -/
theorem mem_of_lin_rel' {K : Subfield ℂ} {u r : ℂ} (hr : r ∈ K) (hr0 : r ≠ 0) {a b : ℂ}
...
/-- **No vanishing coefficient.** -/
theorem no_vanishing_coeff' {K : Subfield ℂ} {u r : ℂ}
...
/-- The double sum is the hand-written coefficient. -/
...
theorem solution
  ...
  have ht : Transcendental (↥Qbar) u := transcendental_candidate_over_base' hu0 hexp
  have huL : u ∉ Qbar := fun hm => ht (isAlgebraic_algebraMap (R := ↥Qbar) ⟨u, hm⟩)
  rw [coeff_eq_matrix']
  exact no_vanishing_coeff' hr huL h w v hwK hvK hw hv
