/-
# The shape of any proof of Diaz's modulus conjecture

Companion to the transfer obstruction `Diaz.candidate_indistinguishable`
(`3592d95c-2f6b-4a36-9888-290e1e3a2c0e`, Proved).  That node produces, for a
hypothetical counterexample `u`, a ring homomorphism `Φ : ℂ → ℂ` which fixes
`Q̄` pointwise, sends `u` to an ordinary circle point `t = r·exp i`, and
commutes with conjugation on `Q̄(u)`.

This file asks what such a `Φ` must *fail* to do, and pins the failures down.
Nothing here advances the conjecture; every result is either a rigidity
statement about ring endomorphisms of `ℂ` or a bookkeeping statement about
which candidate hypotheses survive an arbitrary such `Φ`.

The four failures usually listed for `Φ` — not continuous, not order
preserving, does not commute with `exp`, does not commute with conjugation
outside `Q̄(u)` — are **not four failures**.  Sections 1 and 3 prove:

* continuity, stability of `ℝ`, and global conjugation-compatibility are
  *the same property* for a ring endomorphism of `ℂ`, namely being `id` or
  `conj` (`continuous_iff_id_or_conj`, `mapsTo_real_iff_id_or_conj`,
  `conjComm_iff_id_or_conj`);
* order preservation on `ℝ` is not an extra hypothesis at all: a ring
  endomorphism stabilising `ℝ` is automatically the identity there
  (`eq_self_on_real_of_mapsTo_real`);
* the modulus hypothesis is transferred **verbatim**: `‖Φ u‖ = ‖u‖`
  (`norm_map_eq_of_conjComm`).  So no refinement of "`|u|` is algebraic" —
  realness, positivity, degree, height — can distinguish `u` from `t`;
* a `Φ` that respects *either* the real structure *or* `exp` at `u` carries a
  candidate to a candidate (`isCandidate_map_of_mapsTo_real`,
  `isCandidate_map_of_expComm`).  Hence a transfer map that lands on a
  non-candidate must break **both**, and those two are the only resources a
  proof can use (`transfer_breaks_exactly`).

Section 4 records that `exp`-compatibility alone already pins down a
transcendental constant the algebraic data cannot see: a ring endomorphism
fixing `Q̄` and commuting with `exp` fixes `2πi`, hence `π`
(`map_two_pi_I_of_expComm`).

Section 5 is the reason the obstruction bites at all: the data that `Φ`
preserves — `u ≠ 0`, `u` transcendental, `u · conj u = r²` — is satisfiable in
`ℂ`, so no argument using only that data can derive a contradiction
(`invariant_data_cannot_refute`).  `HermiteLindemann` is taken as an explicit
hypothesis, as elsewhere in this mission, so nothing is assumed silently.

No `sorry`.  Builds against the pinned workspace (Mathlib
...
#print axioms DiazObstructionShape.map_isAlgebraic
#print axioms DiazObstructionShape.eq_self_on_real_of_mapsTo_real
#print axioms DiazObstructionShape.eq_id_or_conj_of_mapsTo_real
#print axioms DiazObstructionShape.mapsTo_real_iff_id_or_conj
#print axioms DiazObstructionShape.conjComm_iff_id_or_conj
#print axioms DiazObstructionShape.continuous_iff_id_or_conj
#print axioms DiazObstructionShape.norm_map_eq_of_conjComm
#print axioms DiazObstructionShape.isCandidate_map_of_expComm
#print axioms DiazObstructionShape.isCandidate_map_of_mapsTo_real
#print axioms DiazObstructionShape.transfer_breaks_exactly
#print axioms DiazObstructionShape.isAlgebraic_I
#print axioms DiazObstructionShape.map_two_pi_I_of_expComm
#print axioms DiazObstructionShape.map_pi_of_expComm
#print axioms DiazObstructionShape.mapsTo_logAlg_of_expComm
#print axioms DiazObstructionShape.transcendental_exp_I
#print axioms DiazObstructionShape.ordinaryPoint_transcendental
#print axioms DiazObstructionShape.ordinaryPoint_mul_conj
#print axioms DiazObstructionShape.exists_ordinary_point
#print axioms DiazObstructionShape.exists_ordinary_point_offAxes
#print axioms DiazObstructionShape.invariant_data_cannot_refute
#print axioms DiazObstructionShape.invariant_data_cannot_refute_offAxes
