...
/-!
# The transcendence-degree-one certificate for the period-aligned `norm_rat_mult` half.

`DZ_ALIGNED_core` reduces that half to the four exponentials conjecture in determinant form.
The published node `DiazModulus.four_exponentials_trdeg_one` is the *theorem* case of that
conjecture: it adds the hypothesis

    Algebra.trdeg ℚ ↥(Algebra.adjoin ℚ {l₁₁, l₁₂, l₂₁, l₂₂}) ≤ 1

and asks for the four entries to be non-zero.  This file supplies both, and so proves the
leaf half from the published node.

The transcendence generator is `x = π i`, **not** `π`: `π` need not lie in the ℚ-algebra
generated by the four entries, while `π i = l₃ / (2c)` does.  Since `π² = −x²`, working over
`ℚ[π i]` costs nothing.
-/
...
theorem transcendental_of_trdegOne (h4 : FourExpTrdegOne) (u : ℂ)
  ...
  have hexpl₃ : IsAlgebraic ℚ (Complex.exp l₃) := by
    rw [hl3_def]; exact DiazAligned.alg_exp_rat_two_pi_I c
...
/-- The rational-multiple half of the aligned leaf, from the published node. -/
theorem alignedRatMult_of_trdegOne (h4 : FourExpTrdegOne) : DiazAligned.AlignedRatMult := by
  intro u _ hnorm _ hax hirr _ hnrm
  obtain ⟨r, _, hβ, c₀, hc⟩ := hnrm
  have hre : u.re ≠ 0 := fun h => hax (Or.inr h)
  exact transcendental_of_trdegOne h4 u hre hirr hnorm (r := r) (c₀ := c₀) hβ hc
