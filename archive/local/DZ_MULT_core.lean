...
/-- Algebraicity is stable under complex conjugation: conjugation is a `ℚ`-algebra map. -/
theorem isAlgebraic_conj {a : ℂ} (h : IsAlgebraic ℚ a) : IsAlgebraic ℚ (conj a) :=
  h.algHom (Complex.conjAe.restrictScalars ℚ).toAlgHom

/-- `ℒ` is stable under conjugation, since `exp (conj u) = conj (exp u)`. -/
theorem logAlg_conj {u : ℂ} (h : u ∈ LogAlg) : conj u ∈ LogAlg := by
  have : Complex.exp (conj u) = conj (Complex.exp u) := Complex.exp_conj u
  rw [LogAlg, Set.mem_setOf_eq, this]
...
/-- Roy's strong six exponentials theorem, carried as an explicit hypothesis:
for `Q̄`-linearly independent `x₁,x₂` and `y₁,y₂,y₃`, at least one of the six
products `xᵢyⱼ` falls outside `ℒ̃`. -/
def StrongSixExponentials : Prop :=
  ∀ (x : Fin 2 → ℂ) (y : Fin 3 → ℂ),
    LinearIndependent (↥Qbar) x → LinearIndependent (↥Qbar) y →
    ¬ (∀ i j, x i * y j ∈ LogAlgTilde)

theorem multiplier_module (hSSE : StrongSixExponentials) (hHL : HermiteLindemann)
    {u : ℂ} (h : IsCandidate u) :
...
#print axioms DiazMult.multiplier_module
