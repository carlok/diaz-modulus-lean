...
/-! ## From Lindemann–Weierstrass to the four nodes

Below, `Complex.exp` is written `Complex.exp`, and `conj` is the complex conjugation of the
preamble's `open ComplexConjugate`. -/

namespace Diaz

/-- **Hermite–Lindemann, contrapositive form.** If `u ≠ 0` and `exp u` is algebraic then `u`
...
/-- Transcendence over `ℚ` upgrades to transcendence over any base algebraic over `ℚ`. -/
...
/-- A candidate is transcendental over the intended base. -/
...
theorem not_on_axes {K : Subfield ℂ} {u : ℂ}
...
theorem four_nodes {K : Subfield ℂ} {u : ℂ}
    (hρ : u * conj u ∈ K) (hre : (u + conj u) ^ 2 ∉ K)
...
/-- If `u + conj u` is transcendental over the base then its square is not in the base. -/
theorem sq_notMem_of_transcendental {L : Subfield ℂ} {u : ℂ}
