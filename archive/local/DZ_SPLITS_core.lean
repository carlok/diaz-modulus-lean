...
/-!
# Splitting `DiazModulus.recip_pi_not_log` along the two coordinate axes of `γ`

Parent (Open):

  `∀ γ : ℂ, IsAlgebraic ℚ γ → γ ≠ 0 → ¬ IsAlgebraic ℚ (exp (γ / (π * I)))`

Write `S₀ := {γ ∈ Q̄ : γ / (iπ) ∈ ℒ}`.  The two facts this file proves are

* `S₀` is closed under `conj` (`axis_conj`), and
* `S₀` is closed under sums and halves (`axis_split`),

so a non-zero element of `S₀` produces a non-zero element of `S₀` that is *either real or purely
imaginary*.  Hence the parent follows from its two restrictions

* `γ.im = 0`   (then `γ/(iπ)` is purely imaginary and `exp` has modulus one), and
* `γ.re = 0`   (then `γ/(iπ)` is real and `exp` is real and positive),

which is a strictly better split than `by_cases γ.im = 0`: the second child gets the *positive*
hypothesis `γ.re = 0` rather than the negative `γ.im ≠ 0`.
-/
