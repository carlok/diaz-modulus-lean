/-
Graph repair C — an alternative reduction of `DiazModulus.diaz_of_exp_not_real`
(`a8b88767-c52e-4166-8a4c-d00348616caa`, Open).

Children:
  * `DiazModulus.diaz_of_exp_not_real_off_axes`  (already in the subgraph, Open leaf — untouched)
  * `DiazModulus.sixExponentials_cannot_refute_candidate`  (Proved, currently unlinked)
  * `DiazModulus.hermite_lindemann_holds`  (already in the subgraph, Proved)

On-axes branch, by dimension.  Clause (2) of the no-go node says that at a candidate, under
Hermite-Lindemann, the certificate space `V = span_Q̄ {1, u, conj u}` has `finrank = 3`.  On
either axis `conj u = ±u`, so `V ≤ span_Q̄ {1, u}`, whose dimension is at most 2.
-/
